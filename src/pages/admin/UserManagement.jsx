...
const UserManagement = () => {
  ...
  // Create axios instance with auth header and better configuration
  const api = axios.create({
    baseURL: API_BASE_URL,
    timeout: 300000, // 5 minutes for all requests
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true)
      const response = await api.get(`${API_URL}/api/admin/users?page=${currentPage}&search=${searchTerm}`)
  ...
  const fetchUserDetails = async (userId) => {
    try {
      const response = await api.get(`${API_URL}/api/admin/users/${userId}/details`)
  ...
  const updateUserRole = async () => {
    try {
      await api.put(`${API_URL}/api/admin/users/${selectedUser._id}/role`, { role: newRole })
  ...
  const deleteUser = async (userId) => {
    if (window.confirm('Are you sure you want to delete this user? This action cannot be undone.')) {
      try {
        await api.delete(`${API_URL}/api/admin/users/${userId}`)
